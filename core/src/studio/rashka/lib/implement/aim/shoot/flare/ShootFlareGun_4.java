...
public class ShootFlareGun_4 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1, x2, y2;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootFlareGun_4() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.add(2, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/FlareGun4.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/FlareGun4u.p"), Gdx.files.internal("particles"));
        fire.get(2).load(Gdx.files.internal("particles/shoot/gun/FlareGun4d.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        rotation(MarsGame.WIDTH / 2 - 65, 540 + 66, 35, -16, position.angle());
        rotation2(MarsGame.WIDTH / 2 - 65, 540 + 66, 35, 16, position.angle());

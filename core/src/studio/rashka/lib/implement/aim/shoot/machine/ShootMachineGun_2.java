...
public class ShootMachineGun_2 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1, x2, y2;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootMachineGun_2() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.add(2, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/MachineGun2.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/MachineGun2u.p"), Gdx.files.internal("particles"));
        fire.get(2).load(Gdx.files.internal("particles/shoot/gun/MachineGun2d.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        rotation(MarsGame.WIDTH / 2 - 265, 540 + 66, 235, -22, position.angle());
        rotation2(MarsGame.WIDTH / 2 - 265, 540 + 66, 235, 25, position.angle());

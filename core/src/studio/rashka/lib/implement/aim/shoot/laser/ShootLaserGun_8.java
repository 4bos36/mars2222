...
public class ShootLaserGun_8 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootLaserGun_8() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/LaserGun8.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/LaserGun8.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        rotation(MarsGame.WIDTH / 2 - 55, 540 + 70, 25, 0, position.angle());

...
public class ShootFlareGun_8 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1, x2, y2, x3, y3;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootFlareGun_8() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.add(2, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/FlareGun8.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/FlareGun8u.p"), Gdx.files.internal("particles"));
        fire.get(2).load(Gdx.files.internal("particles/shoot/gun/FlareGun8d.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        rotation(MarsGame.WIDTH / 2 - 100, 540 + 66, 70, 35, position.angle());
        rotation2(MarsGame.WIDTH / 2 - 100, 540 + 66, 70, -35, position.angle());
        ...
        fire.get(1).getEmitters().first().getAngle().setHigh(150 + position.angle());
        fire.get(1).getEmitters().first().getAngle().setHighMax(300 + position.angle());
        fire.get(2).getEmitters().first().getAngle().setHigh(60 + position.angle());
        fire.get(2).getEmitters().first().getAngle().setHighMax(210 + position.angle());
    }

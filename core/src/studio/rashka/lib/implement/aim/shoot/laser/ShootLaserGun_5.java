...
public class ShootLaserGun_5 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootLaserGun_5() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/LaserGun5.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/LaserGun5.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        rotation(MarsGame.WIDTH / 2 - 60, 540 + 66, 30, 0, position.angle());

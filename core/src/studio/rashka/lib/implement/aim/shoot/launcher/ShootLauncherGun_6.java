...
public class ShootLauncherGun_6 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1, x2, y2;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootLauncherGun_6() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.add(2, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/LauncherGun6.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/LauncherGun6u.p"), Gdx.files.internal("particles"));
        fire.get(2).load(Gdx.files.internal("particles/shoot/gun/LauncherGun6d.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        rotation(MarsGame.WIDTH / 2 - 230, 540 + 66, 200, -40, position.angle());
        rotation2(MarsGame.WIDTH / 2 - 230, 540 + 66, 200, 40, position.angle());

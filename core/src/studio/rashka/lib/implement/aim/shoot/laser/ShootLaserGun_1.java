...
public class ShootLaserGun_1 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootLaserGun_1() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/LaserGun1.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/LaserGun1.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        fire.get(1).getEmitters().first().getRotation().setHigh(180 + position.angle());
    }

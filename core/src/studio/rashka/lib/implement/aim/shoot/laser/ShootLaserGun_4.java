package studio.rashka.lib.implement.aim.shoot.laser;
...
public class ShootLaserGun_4 implements Aims {

    private ArrayList<ParticleEffect> fire;
    private float x1, y1, x2, y2;
    private static final float posCamX = MarsGame.WIDTH / 2, posCamY = 540;

    public ShootLaserGun_4() {
        fire = new ArrayList<ParticleEffect>();

        fire.add(0, new ParticleEffect());
        fire.add(1, new ParticleEffect());
        fire.add(2, new ParticleEffect());
        fire.get(0).load(Gdx.files.internal("particles/shoot/aim/LaserGun4.p"), Gdx.files.internal("particles"));
        fire.get(1).load(Gdx.files.internal("particles/shoot/gun/LaserGun4u.p"), Gdx.files.internal("particles"));
        fire.get(2).load(Gdx.files.internal("particles/shoot/gun/LaserGun4d.p"), Gdx.files.internal("particles"));
    ...
    public void shooting(Vector2 position, Vector2 positionFireAim, OrthographicCamera camera) {
        ...
        rotation(MarsGame.WIDTH / 2 - 195, 540 + 71, 165, -18, position.angle());
        rotation2(MarsGame.WIDTH / 2 - 195, 540 + 66, 165, 17, position.angle());
        ...
        fire.get(1).getEmitters().first().getRotation().setHigh(position.angle());
        fire.get(1).getEmitters().first().getRotation().setHighMax(360 + position.angle());
        fire.get(2).getEmitters().first().getAngle().setHigh(180 + position.angle());
        fire.get(2).getEmitters().first().getRotation().setHigh(position.angle());
        fire.get(2).getEmitters().first().getRotation().setHighMax(360 + position.angle());
    }

...
public class Monster_4 implements Monsters {
    ...
    public Monster_4(Vector2 position, int immunity, int type) {
        ...
        drawer = new Drawer(Textures.getInstance().loader.get("Monster_4"), MarsGame.getBatch(), null);
        immunityMonsters = new ImmunityMonsters("Monster_4", immunity);
    ...
    public void renderDie(SpriteBatch batch) {
        if (MarsGame.getFrustum().sphereInFrustum(position.x, position.y, 0, 25 * scaleSize)) {
            if (!isLiving) batch.draw(Textures.getInstance().textureRegion.get("Monsters_4Die"), position.x - 42 * scaleSize, position.y - 42 * scaleSize, 84 * scaleSize, 84 * scaleSize);
        }
    }

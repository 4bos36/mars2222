...
public class Monster_3 implements Monsters {
    ...
    public Monster_3(Vector2 position, int immunity, int type) {
        ...
        drawer = new Drawer(MarsGame.getTextures().loader.get("Monster_3"), MarsGame.getBatch(), null);
    ...
    public void renderDie(SpriteBatch batch) {
        if (MarsGame.getFrustum().sphereInFrustum(position.x, position.y, 0, 30 * scaleSize)) {
            if (!isLiving) batch.draw(MarsGame.getTextures().textureRegion.get("Monsters_3Die"), position.x - 42 * scaleSize, position.y - 42 * scaleSize, 84 * scaleSize, 84 * scaleSize);
        }
    }

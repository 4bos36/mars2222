...
public class Monster_1 implements Monsters {
  ...
  //  ShapeRenderer renderer;
    ...
    //    renderer = new ShapeRenderer();
        ...
        drawer = new Drawer(Textures.getInstance().loader.get("Monster_1"), MarsGame.getBatch(), null);
    ...
    public void renderDie(SpriteBatch batch) {
        if (MarsGame.getFrustum().sphereInFrustum(position.x, position.y, 0, 30 * scaleSize)) {
            if (!isLiving) batch.draw(Textures.getInstance().textureRegion.get("Monsters_1Die"), position.x - 42 * scaleSize, position.y - 42 * scaleSize, 84 * scaleSize, 84 * scaleSize);
        }
    }
